...
public class CaptchaDialogWidget {
    ...
    public void render(GuiGraphics gui, int x, int y, CaptchaContext captcha) {
        ...
        gui.fill(x - 20, y - 20, x + dialogWidth + 20, y + dialogHeight + 20, OmniTheme.BG_DARK);

        // ---- Panel background ----
        gui.fill(x, y, x + dialogWidth, y + dialogHeight, OmniTheme.BG_PANEL);

        // ---- Double border (classic vanilla) ----
        // Outer: white top/left, dark bottom/right
        gui.hLine(x, x + dialogWidth - 1, y, OmniTheme.BORDER);
        gui.vLine(x, y, y + dialogHeight - 1, OmniTheme.BORDER);
        gui.hLine(x, x + dialogWidth - 1, y + dialogHeight - 1, OmniTheme.BORDER);
        gui.vLine(x + dialogWidth - 1, y, y + dialogHeight - 1, OmniTheme.BORDER);
        // Inner: dark top/left, white bottom/right
        gui.hLine(x + 1, x + dialogWidth - 2, y + 1, OmniTheme.BORDER);
        gui.vLine(x + 1, y + 1, y + dialogHeight - 2, OmniTheme.BORDER);
        gui.hLine(x + 1, x + dialogWidth - 2, y + dialogHeight - 2, OmniTheme.BORDER);
        gui.vLine(x + dialogWidth - 2, y + 1, y + dialogHeight - 2, OmniTheme.BORDER);

        // ---- Title ----
        int titleY = y + DIALOG_PADDING + BORDER;
        gui.drawCenteredString(font, "验证码 (CAPTCHA)", x + dialogWidth / 2, titleY, OmniTheme.TEXT_HEADING_1); // verified: drawCenteredString(Font,String,int,int,int) from lexxie.dev 2026-06-14

        // ---- Subtitle text ----
        int subtitleY = titleY + TITLE_HEIGHT;
        gui.drawCenteredString(font, "请输入验证码以继续搜索", x + dialogWidth / 2, subtitleY, OmniTheme.TEXT_CAPTCHA_SUBTITLE);
