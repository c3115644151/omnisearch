...
class CaptchaDialogWidgetTest {
    ...
    private static final int OVERLAY_ALPHA = 0xCC000000;
    private static final int PANEL_BG = 0xAA1A1A1A;
    ...
    void render_rendersDoubleBorder() {
        ...
        // All borders use dark theme color
        verify(gui).hLine(100, 339, 100, BORDER_COLOR);
        verify(gui).vLine(100, 100, 100 + dialogHeight - 1, BORDER_COLOR);
        verify(gui).hLine(100, 339, 100 + dialogHeight - 1, BORDER_COLOR);
        verify(gui).vLine(339, 100, 100 + dialogHeight - 1, BORDER_COLOR);
        // Inner border
        verify(gui).hLine(101, 338, 101, BORDER_COLOR);
        verify(gui).vLine(101, 101, 100 + dialogHeight - 2, BORDER_COLOR);
        verify(gui).hLine(101, 338, 100 + dialogHeight - 2, BORDER_COLOR);
        verify(gui).vLine(338, 101, 100 + dialogHeight - 2, BORDER_COLOR);
    }
    ...
    void render_rendersTitleAndSubtitle() {
        ...
        verify(gui).drawCenteredString(font, "验证码 (CAPTCHA)", 100 + DIALOG_WIDTH / 2, titleY, TITLE_COLOR);

        int subtitleY = titleY + TITLE_HEIGHT;
        verify(gui).drawCenteredString(font, "请输入验证码以继续搜索", 100 + DIALOG_WIDTH / 2, subtitleY, SUBTITLE_COLOR);
    }
    ...
    void render_rendersCaptchaPlaceholder() {
        ...
        // Placeholder borders (all use OmniTheme.BORDER)
        verify(gui).hLine(placeholderX, placeholderX + CAPTCHA_WIDTH - 1, placeholderY, BORDER_COLOR);
        verify(gui).vLine(placeholderX, placeholderY, placeholderY + CAPTCHA_HEIGHT - 1, BORDER_COLOR);
        verify(gui).hLine(placeholderX, placeholderX + CAPTCHA_WIDTH - 1, placeholderY + CAPTCHA_HEIGHT - 1, BORDER_COLOR);
        verify(gui).vLine(placeholderX + CAPTCHA_WIDTH - 1, placeholderY, placeholderY + CAPTCHA_HEIGHT - 1, BORDER_COLOR);
